import { normalizeRole, hasSystemControlAccess, canManageUsers } from "./access-control.mjs";
...
function applyRolePermissions() {
  ...
  if (manageUsersButton) {
    manageUsersButton.style.display = isAdministrator() ? "" : "none";
  }

  if (deleteHistoryButton) {
    deleteHistoryButton.disabled = !hasSystemControlAccess(role);
    deleteHistoryButton.title = hasSystemControlAccess(role)
      ? ""
      : "Only an Operator or Administrator account can delete history data.";
  }
...
function createUserManagementRow(uid, profile) {
  ...
  row.className = `user-management-row status-${status}`;
  ...
  email.textContent = profile.email || uid;

  const badge = document.createElement("span");
  badge.className = `user-status-badge ${status}`;
  badge.textContent = isCurrentAccount ? "CURRENT ACCOUNT" : status.toUpperCase();
  ...
  roleSelect.setAttribute("aria-label", `Role for ${profile.email || uid}`);
  ...
  statusSelect.setAttribute("aria-label", `Approval status for ${profile.email || uid}`);
  ...
  statusSelect.value = ["pending", "approved", "rejected"].includes(status) ? status : "approved";

  const saveButton = document.createElement("button");
  saveButton.className = "user-access-save";
  saveButton.type = "button";
  saveButton.textContent = "SAVE";
  saveButton.onclick = () => updateUserAccess(uid);

  if (isCurrentAccount) {
    roleSelect.disabled = true;
    statusSelect.disabled = true;
    saveButton.disabled = true;
    saveButton.title = "You cannot modify your own administrator access here.";
  }

  controls.append(roleSelect, statusSelect, saveButton);
...
async function loadUserManagement() {
  ...
  try {
    ...
    const entries = Object.entries(users).sort(([, a], [, b]) => {
      ...
      const statusDifference = (statusOrder[getProfileStatus(a)] ?? 3) - (statusOrder[getProfileStatus(b)] ?? 3);
      if (statusDifference !== 0) return statusDifference;
      return String(a.name || a.email || "").localeCompare(String(b.name || b.email || ""));
    });

    const pendingCount = entries.filter(([, profile]) => getProfileStatus(profile) === "pending").length;
    summary.textContent = `${entries.length} user${entries.length === 1 ? "" : "s"} • ${pendingCount} pending approval`;

    if (!entries.length) {
      const empty = document.createElement("div");
      empty.className = "user-management-empty";
      empty.textContent = "No user profiles found in Firebase Database.";
      list.append(empty);
      return;
    }

    entries.forEach(([uid, profile]) => {
      list.append(createUserManagementRow(uid, profile || {}));
    });
  } catch (error) {
    console.error("Unable to load users", error);
    summary.textContent = "Unable to load users.";
    const errorMessage = document.createElement("div");
    errorMessage.className = "user-management-empty error";
    errorMessage.textContent = "Check that the current account is an Administrator and that Firebase Database rules allow Administrators to read /users.";
...
window.updateUserAccess = async function(uid) {
  ...
  const saveButton = roleSelect?.parentElement?.querySelector(".user-access-save");
...
window.deleteAllHistory = async function() {
  if (!isAdmin()) {
    alert("Only operator or administrator can delete history data.");
    return;
  }
