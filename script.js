...
function applyRolePermissions() {
  ...
  if (manageUsersButton) {
    manageUsersButton.style.display = role === "admin" ? "" : "none";
  }

  if (deleteHistoryButton) {
    deleteHistoryButton.disabled = role !== "admin";
    deleteHistoryButton.title = role === "admin"
      ? ""
      : "Only an admin account can delete history data.";
  }
...
async function loadUserManagement() {
  ...
  try {
    ...
    errorMessage.textContent = "Check that the current account is an admin and that Firebase Database rules allow admins to read /users.";
...
window.openUserManagement = function() {
  if (!isAdmin()) {
    alert("Only admin can manage user accounts.");
    return;
  }
...
window.updateUserAccess = async function(uid) {
  ...
  const role = roleSelect.value === "admin" ? "admin" : "viewer";
  ...
  try {
    ...
    if (status === "approved") {
      ...
      changes.approvedBy = auth.currentUser?.email || auth.currentUser?.uid || "admin";
    } else {
