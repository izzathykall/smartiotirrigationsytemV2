...
// Firebase Authentication + user approval system
...
window.login = async function() {
  ...
  try {
    ...
    const snap = await get(ref(database,"users/"+result.user.uid));
    if(!snap.exists() || snap.val().approved !== true){
      await auth.signOut();
    ...
    sessionStorage.setItem("smartIrrigationUser",email);
    document.body.classList.add("logged-in");
    applyRolePermissions();
    loadAdminPanel();
  } catch(e){ errorBox.innerHTML=e.message; }
};
...
window.registerUser = async function(){
 ...
 try{
  ...
  await set(ref(database,"users/"+r.user.uid),{email,approved:false,role:"viewer"});
...
};port { initializeApp } from "https://www.gstatic.com/firebasejs/12.0.0/firebase-app.js";
import { getDatabase, ref, onValue, set, get } from "https://www.gstatic.com/firebasejs/12.0.0/firebase-database.js";
import { getAuth, signInWithEmailAndPassword, createUserWithEmailAndPassword, sendPasswordResetEmail } from "https://www.gstatic.com/firebasejs/12.0.0/firebase-auth.js";
...
window.loadAdminPanel=function(){
 ...
 onValue(ref(database,"users"),snap=>{
  ...
  snap.forEach(u=>{
    ...
    if(d.approved!==true){
      out+=`<p>${d.email} 
      <button onclick="approveUser('${u.key}','viewer')">Viewer</button>
      <button onclick="approveUser('${u.key}','full')">Full Access</button></p>`;
    }
  });
  document.getElementById("pendingUsers").innerHTML=out||"No pending users";
 });
};

window.approveUser=async function(uid,role){
 await set(ref(database,"users/"+uid+"/approved"),true);
