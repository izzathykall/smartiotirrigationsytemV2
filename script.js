...
import { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, sendPasswordResetEmail, signOut } from "https://www.gstatic.com/firebasejs/12.0.0/firebase-auth.js";
...
window.login = async function() {
  ...
  try {
    ...
    sessionStorage.setItem("smartIrrigationRole","viewer");
...
window.registerAccount = async function(){
 ...
 try {
  ...
  await set(ref(database,"users/"+r.user.uid),{email:email,status:"pending",role:"viewer"});
...
window.resetPassword=async function(){
 const email=document.getElementById("loginUsername").value.trim();
 try{await sendPasswordResetEmail(auth,email);alert("Reset password email sent.");}
 catch(e){alert(e.message);}
};
