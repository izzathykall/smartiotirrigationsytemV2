...
async function resolveUserRole(user) {
  try {
    ...
    if (role === "admin" || role === "administrator" || role === "fullaccess") {
      return "admin";
    }

    return "viewer";
  } catch (error) {
    console.warn("Unable to read user role. Defaulting to viewer.", error);
    return "viewer";
  }
}
...
onAuthStateChanged(auth, async user => {
  ...
  // Ignore stale results if the account logged out while the role was loading.
