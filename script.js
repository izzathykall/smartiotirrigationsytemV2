...
import { getAuth, signInWithEmailAndPassword, createUserWithEmailAndPassword, sendPasswordResetEmail, signOut } from "https://www.gstatic.com/firebasejs/12.0.0/firebase-auth.js";
  ...
  try {
    ...
    onValue(ref(database,"users/"+user.uid),(snap)=>{
      ...
      if(!data || data.status !== "approved"){
        errorBox.innerHTML="Account waiting for admin approval";
        signOut(auth);
        return;
      }

      sessionStorage.setItem("smartIrrigationLogin","true");
      sessionStorage.setItem("smartIrrigationRole",data.role);
      sessionStorage.setItem("smartIrrigationUser",user.email);
...
window.register = async function(){
  ...
  try{
    ...
    msg.innerHTML="Registration successful. Wait for admin approval.";
 ...
 onValue(ref(database,"users"),snap=>{
   ...
   snap.forEach(item=>{
    ...
    if(u){
     ...
     <button onclick="approveUser('${item.key}')">Approve Viewer</button>
     <button onclick="setRole('${item.key}','fullaccess')">Full Access</button>
     <button onclick="setRole('${item.key}','admin')">Admin</button>
     </div>`;
    }
   });
 });
...
window.approveUser=async function(uid){
 await update(ref(database,"users/"+uid),{status:"approved",role:"viewer"});
}

window.setRole=async function(uid,role){
 await update(ref(database,"users/"+uid),{status:"approved",role:role});
}
