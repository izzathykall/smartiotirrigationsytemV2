  ...
  try {
    ...
    if (permission === "granted") {
      ...
      if (token) {
        ...
        // Keep only the current token for this user. Old tokens can remain valid
        // after a PWA reinstall and otherwise cause duplicate notifications.
        const tokenListRef = ref(database, `users/${userUid}/fcmTokens`);
        const tokenSnapshot = await get(tokenListRef);
        const tokenUpdates = {
          [`users/${userUid}/fcmTokens/${token}`]: true
        };

        tokenSnapshot.forEach(childSnapshot => {
          if (childSnapshot.key !== token) {
            tokenUpdates[`users/${userUid}/fcmTokens/${childSnapshot.key}`] = null;
          }
        });

        await update(ref(database), tokenUpdates);
      }
    }
  } catch (error) {
